...
/* Logo fallback */
(function () {
  const img = qs("#peerlistLogo"),
    fallback = qs("#peerlistLogoFallback");
  if (!img) return;
...
async function openFromCard(card) {
  ...
  if (canVT) {
    ...
    media.classList.add("vt-source");
    hero.classList.add("vt-target");
    media.style.viewTransitionName = vtName;
    hero.style.viewTransitionName = vtName;
    ...
    media.classList.remove("vt-source");
    hero.classList.remove("vt-target");
...
async function openFLIP(card, media) {
  ...
  page.classList.add("active");
  page.style.opacity = 0; // fade later
  ...
  const fade = page.animate([{ opacity: 0 }, { opacity: 1 }], {
    duration: 220,
    delay: 120,
    easing: "linear",
  });
...
async function closeFLIP(card, media) {
  ...
  const fade = page.animate([{ opacity: 1 }, { opacity: 0 }], {
    duration: 180,
    easing: "linear",
  });
  ...
  try {
    if (navigator.share) {
      await navigator.share({
        ...
        text: "Card → Page (Polished Minimal)",
        url,
      });
    } else {
